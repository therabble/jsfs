...
function balanced_storage_target_indexes() {
      ...
      if (balanced_storage_location_indexes.length <= config.MINIMUM_NONMIRROR_LOCATIONS_FOR_A_BLOCK) {
        //we're writing to every balanced, so return entire list
        return balanced_storage_location_indexes;
      }
      var returnIndexes = [];
      for (let j = 0; j < config.MINIMUM_NONMIRROR_LOCATIONS_FOR_A_BLOCK; j++) {
