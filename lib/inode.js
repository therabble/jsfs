...
var balanced_storage_location_indexes = [];
for (let i = 0; i < config.STORAGE_LOCATIONS.length; i++) {
  if (config.STORAGE_LOCATIONS[i].status === "balanced") {
    balanced_storage_location_indexes.push(i);
  }
}
var next_balanced_storage = 0;

function balanced_storage_target_indexes() {
      // increment (or reset) storage location (striping)
      if (balanced_storage_location_indexes.length == 0) {
        //there are none, returning empty list
        return [];
      }
      if (balanced_storage_location_indexes.length <= config.MINIMUM_BALANCED_LOCATIONS_FOR_A_BLOCK) {
        //we're writing to every balanced, so return entire list
        return balanced_storage_location_indexes;
      }
      var returnIndexes = [];
      for (let j = 0; j < config.MINIMUM_BALANCED_LOCATIONS_FOR_A_BLOCK; j++) {
        returnIndexes.push(balanced_storage_location_indexes[next_balanced_storage]);
        next_balanced_storage++;
        if(next_balanced_storage === balanced_storage_location_indexes.length){
          next_balanced_storage = 0;
        }
      }
...
var Inode = {
  ...
  write: function(chunk, req, callback){
    ...
    if (this.block_size && this.input_buffer.length > this.block_size) {
      req.pause();
  ...
  store_block: function(update_file_size, callback){
    ...
    var block;
    if (chunk_size == 0) {
      block = this.input_buffer;
      log.message(log.DEBUG, "chunk_size was zero, so block is whole input buffer");
    } else {
      block = this.input_buffer.slice(0, chunk_size);
      log.message(log.DEBUG, "chunk_size was " + chunk_size + ".");
    }
    ...
    utils.commit_block_to_disk(block, block_object, balanced_storage_target_indexes(), function(err, result){
      if (err) {
        return callback(err);
      }
